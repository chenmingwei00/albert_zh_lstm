  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        precision,precision_update_op= tf.metrics.precision(labels=label_ids,predictions=predictions,weights=is_real_example)
        recall,recall_update_op= tf.metrics.recall(labels=label_ids,predictions=predictions,weights=is_real_example)
     ...
     output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          predictions={"probabilities": predictions,'text_id':text_id},
          scaffold_fn=scaffold_fn)
    return output_spec
